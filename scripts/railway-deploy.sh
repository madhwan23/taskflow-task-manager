...
railway variable --service backend --set "ALLOWED_HOSTS=.up.railway.app" --skip-deploys
railway variable --service backend --set "CORS_ALLOWED_ORIGINS=https://placeholder.up.railway.app" --skip-deploys
...
railway variable --service frontend --set "NIXPACKS_NODE_VERSION=22" --skip-deploys

railway up "$ROOT/backend" --path-as-root --service backend --detach
BACKEND_DOMAIN="$(railway domain --service backend --port 8000 | grep -Eo '[a-zA-Z0-9.-]+\\.up\\.railway\\.app' | tail -n 1)"
...
FRONTEND_DOMAIN="$(railway domain --service frontend --port 4173 | grep -Eo '[a-zA-Z0-9.-]+\\.up\\.railway\\.app' | tail -n 1)"
